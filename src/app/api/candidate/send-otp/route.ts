...
export async function POST(req: NextRequest) {
  try {
    ...
    const emailResult = await sendEmail({
      to: cleanEmail,
      subject: `Your Candidate Verification OTP: ${otp}`,
      html: `
        <div style="font-family: Arial, sans-serif; background-color: #f4f4f5; padding: 32px; border-radius: 8px; max-width: 520px; margin: 0 auto;">
          <div style="text-align: center; margin-bottom: 24px;">
            <h2 style="color: #f97316; margin: 0; font-size: 22px;">Redlix Secure</h2>
            <p style="color: #71717a; font-size: 13px; margin-top: 4px;">Candidate Account Email Verification</p>
          </div>
          <div style="background-color: #ffffff; padding: 24px; border-radius: 8px; border: 1px solid #e4e4e7; text-align: center;">
            ...
            <div style="background-color: #fff7ed; border: 1.5px dashed #f97316; display: inline-block; padding: 14px 28px; font-size: 32px; font-weight: 800; letter-spacing: 6px; color: #f97316; border-radius: 6px; margin: 16px 0;">
              ${otp}
            </div>
    ...
    console.error("POST /api/candidate/send-otp error:", error);
